...
def validate(val_loader, model, criterion, num_classes, args, flip=False, test_batch=6):
    ...
    animal_pck_score = './animal_{}_pck_score.npy'.format(args.animal)
    with open(animal_pck_score, 'wb') as f:
        np.save(f, np.array(preds_list))
    parts = {'eye': [0, 1], 'chin': [2], 'hoof': [3, 4, 5, 6], 'knee': [8, 9, 10, 11],
             'elbow': [14, 15, 16, 17]}
    score_list = []
    for p in parts.keys():
        part = parts[p]
    ...
    print("mean {}".format(statistics.mean(score_list)))
